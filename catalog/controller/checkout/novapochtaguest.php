...
class ControllerCheckoutNovapochtaguest extends Controller {
  	public function index() {
    	$this->language->load('checkout/novapochtacheckout');
        ...
        $this->data['entry_otdel'] = $this->language->get('entry_otdel');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/novapochtaguest.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/checkout/novapochtaguest.tpl';
		} else {
			$this->template = 'default/template/checkout/novapochtaguest.tpl';
		}
	...
	public function validate() {
		...
		if (!$json) {
			...
			if ((utf8_strlen($this->request->post['city']) < 3)) {
				$json['error']['city'] = $this->language->get('error_novaposhta_city');
			}

			if (utf8_strlen($this->request->post['address_1']) < 1) {
				$json['error']['address_1'] = $this->language->get('error_novaposhta_address_1');
			}
