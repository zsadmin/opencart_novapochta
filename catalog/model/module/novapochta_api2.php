...
class NovaPoshtaApi2 {
    ...
    function getArea($findByString = '', $ref = '') {
        ...
        empty($this->areas) AND $this->areas = include dirname(__FILE__) . '/novapochta_api2_areas.php';
        $error = '';
    ...
    function newInternetDocument($sender, $recipient, $params) {
        ...
        if ( !isset($sender['Sender']) ) {
            ...
            $senderCounterpartyExisting = $this->getCounterparties('Sender', 1, '', '');
        ...
        if ( !isset($recipient['Recipient']) ) {
	        ...
	        if (isset($recipientCounterparty['data'][0])) {
    ...
    /**
     * Delete document by ref
     *
     * @param $ref
     * @return mixed
     */
    function deleteInternetDocument($ref){
        return $this->model('InternetDocument')->delete(array('DocumentRefs' => $ref));
    }
